...
if( $_SESSION['hisseall']==0){
    echo "Les séances effectuées le ".format($_SESSION['datbs']) ;
}
if( $_SESSION['hisseall']==1){
 ...
 echo "Les séances effectuées le ".format($_SESSION['datbs']) ;
    ...
    if( $_SESSION['hissepar']!=0){
    echo "Les séances effectuées le ".format($_SESSION['datbs'])." par ".completname($_SESSION['hissepar']);
    }
}
       }
...
if( $_SESSION['hisseall']==0){
    echo "Mes séances effectuées le ".format($_SESSION['datbs']) ;
}
if( $_SESSION['hisseall']==1){
  if( $_SESSION['hissepar']==0){
 echo "Mes séances effectuées le ".format($_SESSION['datbs']) ;
...
<?php  
 if(!isset($_SESSION['wegonlinenum'])){ 
if( $_SESSION['hisseall']==0){
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee WHERE seance.dateseance='".$_SESSION['datbs']."'  ORDER BY idseance DESC ";
}
if( $_SESSION['hisseall']==1){
  if( $_SESSION['hissepar']==0){
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee WHERE seance.dateseance='".$_SESSION['datbs']."'   ORDER BY idseance DESC ";   
  ...
  if( $_SESSION['hissepar']!=0){
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee WHERE seance.dateseance='".$_SESSION['datbs']."'  AND 	seance.idenseign='".$_SESSION['hissepar']."' ORDER BY idseance DESC ";
  }
}
 }
...
<nav class="main-header navbar navbar-expand navbar-dark">
  <footer class="main-footer">
   <div class="btn-group w-100 mb-2">
    <?php   echo"<a class='btn btn-success' type='button' data-toggle='modal'   data-target='#iseanc'><small> Choisir une date</small></a>";
if(!isset($_SESSION['wegonlinenum'])){
  ...
  <div class="modal-body">
...
</br><b>Les séances effectués sur une date</b></br>
   ...
   <form action='seancejr.php' method='POST'>
    ...
    echo "<div class='btn-group w-100 mb-2'>
     <button type='submit' name='envoipe2' class='btn btn-success'>  Valider</button>
   </div>";
