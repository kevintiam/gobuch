...
if( $_SESSION['hisseall']==0){
    echo "Toutes les séances effectuées ";
}
if( $_SESSION['hisseall']==1){
 ...
 echo "Toutes les séances effectuées ";
    ...
    if( $_SESSION['hissepar']!=0){
    echo "Toutes les séances effectuées par ".completname($_SESSION['hissepar']);
    }
}
      }
...
if( $_SESSION['hisseall']==0){
    echo "Toutes mes séances effectuées ";
}
if( $_SESSION['hisseall']==1){
  if( $_SESSION['hissepar']==0){
 echo "Toutes mes séances effectuées ";
...
$v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee   ORDER BY idseance DESC ";
  ...
  if( $_SESSION['hissepar']!=0){
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee WHERE seance.idenseign='".$_SESSION['hissepar']."' ORDER BY idseance DESC ";
  }
}
 }  
...
<nav class="main-header navbar navbar-expand navbar-dark">
  <footer class="main-footer">
   <div class="btn-group w-100 mb-2">
    <?php  if(!isset($_SESSION['wegonlinenum'])){
         echo" 
                       <a class='btn btn-secondary' type='button' data-toggle='modal'   data-target='#tuteur'><small>Trier par tuteur.e</small></a>
  ";  
    }   
