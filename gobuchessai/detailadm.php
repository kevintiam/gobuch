...
$_SESSION['nomprgm']=1;  $_SESSION['backemp']=0;
...
<head>
  ...
  <title>gobuch - organiser l'emploi du temps</title>
...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-dark">
    ...
    <ul class="navbar-nav">
     ...
     <li class="nav-item">
      <?php  echo "<form action='demandeacc.php' method='POST'>"; ?>
     <button type="submit" class="btn"  style='color:white';>
      <i class="fas fa-arrow-left text-muted"></i>
    </button>
     </form> 
...
<?php 
   ...
   $idbenutzer=$donnees['idutilisateur'];
    ...
    <div class='col-12 col-sm-6 col-md-4 d-flex align-items-stretch flex-column'>
     ...
     Numéro whatsapp du demandeur : <br><a href='https://wa.me/".$numbwh."' style='color:black';>".$numbwh." &nbsp &nbsp &nbsp <span class='fab fa-whatsapp' style='color:green';></span></a></br> <br> 
            ...
            <!--a href='depensedetail.php?ID=63' class='nav-link'-->
             <div class='card bg-light d-flex flex-fill'>
                 <div class='card-header border-bottom-0'>
                  ...
                  <b>Genre souhaité : ".genre($genre)."<br>
                  </b>
                   <br><br><br>
                   ";
  ...
  ?>
   ...
   <?php
    ...
    <button type='submit' class='btn btn-success' value='send'>Votre demande a été enregistrée</button>
      ...
      ?>
<div class='card card-solid'>
  <div class='card-body pb-0'><b>Emploi du temps
    <a type='button' data-toggle='modal'  style='color:green'; data-target='#newcompet'><i class='nav-icon fas fa-plus' ></i> Ajouter un nouvel emploi</a>
    

 <?php   echo "<div class='modal fade' id='newcompet'> " ; ?>  
  <div class="modal-dialog">
    <div class="modal-content ">
      <div class="modal-header">
       <h4 class="modal-title">Editer l'emploi du temps -  <?php echo nomclasse($dclasse);?>  </h4> 
  ...
  <div class="modal-body">
  <form action='executereq.php' method='POST' name='emploi1' oninput='conclure1()' enctype='multipart/form-data'>
  <?php 
  ...
  echo "<div class='select2-purple'> 
        ...
        while ($donmat = $repmat->fetch() )
          {
        echo "<option value='".$donmat['idmatiere']."'>".nomatiere($donmat['idmatiere'])."</option>";
           }
          $repmat->closeCursor();
           echo "</select></div><br>";

        echo "<input style='display:none'; name='clax' value='".$dclasse."'   class='form-control'></br> ";

         /*    echo "<div class='select2-purple'>
              <label for='exampleInputPassword1'>Tuteur.e</label>
              <select name='tute'    class='select2' data-dropdown-css-class='select2-purple' style='width: 100%;'> "; 
               
              $seqreq="SELECT * FROM ensmatclas";
           $repseq = $bdd->query($seqreq);
             
          while ($donseq = $repseq->fetch() )
          {
            $iusempl=$donseq['idutilisateur'];
            $iusecl=$donseq['idclas'];
            if($donseq['idmat']==$idmatiere AND $donseq['idclas']==$dclasse){
              $ligneinfo=infokontouser($iusempl);
              $tabe=array();
               $tabe=explode("@",$ligneinfo);
              echo "<option value='".$iusempl."'>".$tabe[0]." ".$tabe[1]."</option>";
            }
...
echo "<label for='exampleInputPassword1'> Taux horaire en FCFA <i>(A payer par le demandeur)</i></label><input name='taux' id='tau' class='form-control'></br> ";
 echo "<label for='exampleInputPassword1'> Montant total en FCFA <i>(A payer par le demandeur)</i></label><input name='total' id='tot' class='form-control'></br> ";
 
 echo "<label for='exampleInputPassword1'> Taux horaire en FCFA <i>(A recevoir par le tuteur)</i></label><input name='tauxtu' id='tautu' class='form-control'></br> ";
 echo "<label for='exampleInputPassword1'> Montant total en FCFA <i>(A recevoir par le tuteur)</i></label><input name='totaltu' id='totu' class='form-control'></br> ";
      ...
      <button type="submit" name="addnewemp" value="<?php echo $idemande; ?>" class="btn btn-primary">Enregistrer</button>
        </form></div>
    </div>
    <!-- /.modal-content -->
  </div>
...
<!-- /.modal -->  
        ...
        $lsql="SELECT * FROM estsollicitee  WHERE idemande='".$_SESSION['nodembetreuer']."'  ORDER BY jourcours";
           $reponse = $bdd->query($lsql);
           $cpt=0;  
          while ($donnees = $reponse->fetch() )
          {
...
// data-target='".$mode."'
     echo "<div class='col-12 col-sm-6 col-md-4 d-flex align-items-stretch flex-column' style='color:black';> <div class='card card-solid'>
      ...
      Jour : <span class='badge bg-success'>".nomjour($ijourcours)."</span> <br> 
       ...
       Durée : <span class='badge bg-primary'>".nomduree($duree)." heure.s</span><br>
...
<?php   echo "<div class='modal fade' id='".$mode1."'> " ; ?>
  ...
  <?php 
      ...
      <button type="submit" name="supperiodem" value="<?php echo $idestsollicitee; ?>" class="btn btn-primary">Supprimer</button>
        </form></div>
    </div>
    <!-- /.modal-content -->
  </div>
...
<?php   echo "<div class='modal fade' id=' '> " ; ?>
  <div class="modal-dialog">
    <div class="modal-content ">
      <div class="modal-header">
         <h4 class="modal-title">  cette plage </h4>  
  ...
  <div class="modal-body">
  <?php   echo "<form action='executereq.php' method='POST'   oninput='".$conclur."' enctype='multipart/form-data'>"; ?>
  <?php 
  /* $idenseign=$donnees['idenseign'];
            $idmatiere=$donnees['idmatiere'];
            $duree=$donnees['duree'];
  ...
  echo "<div class='select2-purple'> 
              <label for='exampleInputPassword1'>Jour</label>
              <select name='day' id='day".$cpt."'  class='select2' data-dropdown-css-class='select2-purple' style='width: 100%;'>  
        ...
        <option value='7'>Dimanche</option>";
            ...
            echo "<div class='select2-purple'>
              <label for='exampleInputPassword1'>Heure de debut</label>
              <select name='debut'  id='debut".$cpt."'  class='select2' data-dropdown-css-class='select2-purple' style='width: 100%;'> "; 
        ...
        <option value='12'>18:00</option><option value='13'>18:30</option><option value='14'>19:00</option>
            ...
            echo "<div class='select2-purple'>
              <label for='exampleInputPassword1'>Durée</label>
              <select name='duree' id='durer".$cpt."'  class='select2' data-dropdown-css-class='select2-purple' style='width: 100%;'> "; 
        ...
        <option value='6'>06:00</option>
            ...
            echo "<div class='select2-purple'>
              <label for='exampleInputPassword1'>Matière</label>
              <select name='matie' id='matie".$cpt."'   class='select2' data-dropdown-css-class='select2-purple' style='width: 100%;'> "; 
  ...
  echo "<div class='select2-purple'>
              <label for='exampleInputPassword1'>Cliquer pour faire la recherche</label>
              <select name='ensb' id='ensb".$cpt."'  onChange='trituteur(this.value)' class='select2' data-dropdown-css-class='select2-purple' style='width: 100%;'> "; 
     echo "<option value=' '>  </option>";
   echo "<option value=' '> Actualiser la recherche</option>";
     echo "</select></div><br>";
...
echo "<label for='exampleInputPassword1'> Taux horaire en FCFA <i>(A payer par le demandeur)</i></label><input name='taux' id='tau".$cpt."' value='".$th."' class='form-control'></br> ";
 echo "<label for='exampleInputPassword1'> Montant total en FCFA <i>(A payer par le demandeur)</i></label><input name='total' id='tot".$cpt."' value='".$montant."' class='form-control'></br> ";
 
 echo "<label for='exampleInputPassword1'> Taux horaire en FCFA <i>(A recevoir par le tuteur)</i></label><input name='tauxtu' value='".$thens."'  id='tautu".$cpt."' class='form-control'></br> ";
 echo "<label for='exampleInputPassword1'> Montant total en FCFA <i>(A recevoir par le tuteur)</i></label><input name='totaltu' value='".$montantens."'  id='totu".$cpt."' class='form-control'></br> ";
 ...
 echo "<div class='select2-purple'>
            ...
            echo "<option value='".$decision."'>".actifemploi($decision)."</option>
              ...
              <option value='1'>Norn actif</option> ";
  ...
  <?php 
      ...
      <button type="submit" name="paramdmdserv" value="<?php echo $idemande; ?>" class="btn btn-primary">Enregistrer</button>
        </form></div>
    </div>
    <!-- /.modal-content -->
  </div>
...
Les séances effectuées pour cette demande
</b> 
      </div></div>
...
<?php   
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee  ORDER BY idseance DESC ";
	 $reponse = $bdd->query($v);
 
	while ($donnees = $reponse->fetch() )
	{
	      if($donnees['idemande']==$idemande){ 
    ...
    $dureecou=calculduree($heuredeb,$uhr); $seconde=date('s');
 if($decision==1){ $dureehm=$dureecou.":".$seconde; $decsean="<span class='badge bg-warning'>En cours</span>";}    if($decision==2){ $decsean="<span class='badge bg-secondary'>Terminée</span>";}  
...
<small>Numéro de l'emploi du temps : ".$idestsollicitee."</small></br>";
