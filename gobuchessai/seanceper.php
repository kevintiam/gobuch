...
if(isset($_POST['envoipe'])){
  ...
  $_SESSION['hisseall']=0; // TOUT LE MONDE
  $_SESSION['hissepar']=0;
  $aasd=$_POST['datpdeb'];
  $pa=array();
  $pa=explode("-",$aasd);
  $_SESSION['datbs']=$pa[0].".".$pa[1].".".$pa[2];

  $fasd=$_POST['datpfin'];
  $fa=array();
  $fa=explode("-",$fasd);
  $_SESSION['datfs']=$fa[0].".".$fa[1].".".$fa[2];
}

if(isset($_POST['envoipe2'])){
  // $_SESSION['datebesuch']=$_POST['datedeb'];
...
if(isset($_POST['envoipe3'])){
  // $_SESSION['datebesuch']=$_POST['datedeb'];
    $_SESSION['hisseall']=1; // par perso
   $_SESSION['hissepar']=$_POST['idensei'];
...
if(isset($_POST['envoipe'])){
  // $_SESSION['datebesuch']=$_POST['datedeb'];
  $_SESSION['hisseall']=1; // TOUT LE MONDE
  $_SESSION['hissepar']=$_SESSION['wegonlinenum'];
  $aasd=$_POST['datpdeb'];
  $pa=array();
  $pa=explode("-",$aasd);
  $_SESSION['datbs']=$pa[0].".".$pa[1].".".$pa[2];
...
<head>
  ...
  <title>gobuch - les séances</title>
...
<div class='card card-solid'>
  <div class='card-body pb-0'><b> 
    <?php  
    if(!isset($_SESSION['wegonlinenum'])){
if( $_SESSION['hisseall']==0){
    echo "Les séances effectuées du ".format($_SESSION['datbs'])." au ".format($_SESSION['datfs']);
}
if( $_SESSION['hisseall']==1){
  if( $_SESSION['hissepar']==0){
 echo "Les séances effectuées du ".format($_SESSION['datbs'])." au ".format($_SESSION['datfs']);
  }
    if( $_SESSION['hissepar']!=0){
    echo "Les séances effectuées du ".format($_SESSION['datbs'])." au ".format($_SESSION['datfs'])." par ".completname($_SESSION['hissepar']);
    }
}
    }

      if(isset($_SESSION['wegonlinenum']) AND $_SESSION['wegonlinenum']!=0){
if( $_SESSION['hisseall']==0){
    echo "Mes séances effectuées du ".format($_SESSION['datbs'])." au ".format($_SESSION['datfs']);
}
if( $_SESSION['hisseall']==1){
  if( $_SESSION['hissepar']==0){
 echo "Mes séances effectuées du ".format($_SESSION['datbs'])." au ".format($_SESSION['datfs']);
...
<?php   
 if(!isset($_SESSION['wegonlinenum'])){
if( $_SESSION['hisseall']==0){
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee WHERE seance.dateseance>='".$_SESSION['datbs']."'  AND seance.dateseance<='".$_SESSION['datfs']."' ORDER BY idseance DESC ";
}
if( $_SESSION['hisseall']==1){
  if( $_SESSION['hissepar']==0){
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee WHERE seance.dateseance>='".$_SESSION['datbs']."'  AND seance.dateseance<='".$_SESSION['datfs']."' ORDER BY idseance DESC ";   
  ...
  if( $_SESSION['hissepar']!=0){
 $v="SELECT * FROM seance INNER JOIN estsollicitee ON seance.idestsollicitee=estsollicitee.idestsollicitee WHERE seance.dateseance>='".$_SESSION['datbs']."'  AND seance.dateseance<='".$_SESSION['datfs']."' AND 	seance.idenseign='".$_SESSION['hissepar']."' ORDER BY idseance DESC ";
  }
}
 }
...
<small>Numéro de l'emploi du temps : ".$idestsollicitee."</small></br>";
      ...
      echo " </b>"; 
       ...
       <button class='btn btn-danger' type='button' data-toggle='modal'   data-target=' '><i class='fas fa-trash'></i></button>          
...
<nav class="main-header navbar navbar-expand navbar-dark">
  <footer class="main-footer">
   <div class="btn-group w-100 mb-2">
    <?php   echo"<a class='btn btn-success' type='button' data-toggle='modal'   data-target='#iseanc'><small> Choisir une période</small></a>";
if(!isset($_SESSION['wegonlinenum'])){
echo "<a class='btn btn-secondary' type='button' data-toggle='modal'   data-target='#tuteur'><small>Trier par tuteur.e</small></a>
  ";  
}

 echo " <a class='btn btn-primary' type='button' data-toggle='modal'><small>Somme : ".$somme." FCFA</small></a>";  
     ?>  
    </div>
   </footer>
   </nav>
 ...
 <form action='seanceper.php' method='POST'>
  ...
  <input value='Au' disabled='disabled'  class='form-control'></div>
    ...
    echo "<div class='btn-group w-100 mb-2'>
     <button type='submit' name='envoipe2' class='btn btn-secondary'>  Valider</button>
   </div>";
...
<!-- /.modal -->
  ...
  <?php   echo "<div class='modal fade' id='tuteur'> " ; ?>
  <div class="modal-dialog">
    <div class="modal-content ">
      <div class="modal-header">
        <h4 class="modal-title">Les séances effectuées</h4>
...
<?php 
echo "</br><b>Choisir le/la tuteur.e</b></br>
 <form action='seanceper.php' method='POST'>";
    echo "<div class='select2-purple'>
    <select name='idensei'  class='select2' data-dropdown-css-class='select2-purple' style='width: 100%;'>";
    echo "<option value='0'>Tout</option>" ;
    $chap="SELECT * FROM enseignant INNER JOIN utilisateur ON utilisateur.idutilisateur=enseignant.idutilisateur  ORDER BY nom ";
  $rep = $bdd->query($chap);
  while ($don = $rep->fetch()){
      echo "<option value='".$don['idutilisateur']."'>".completname($don['idutilisateur'])."</option>" ;
    } 
    $rep->closeCursor();  
   echo "</select>
  </div></br>
    ...
    echo "<div class='btn-group w-100 mb-2'>
     <button type='submit' name='envoipe3' class='btn btn-secondary'>  Valider</button>
   </div>";
